...
sudo docker exec -it $DOCKER_IMAGE make -j$(nproc)
sudo docker exec -it $DOCKER_IMAGE checkinstall -D -y --nodoc --backup=no --pkglicense="GPL2" --maintainer="S. Yakupov \\<s.yakupov@noviga.com\\>" --requires="libasound2,libgig9,jackd,libsndfile1,libsqlite3-0" --install=no
